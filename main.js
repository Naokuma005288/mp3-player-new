...
const ui = {
  ...
  // optional advanced buttons (存在しなければnull)
...
// audioBが無ければ作ってappendしておく（クロスフェード用）
if (!ui.audioB) {
...
ui.eqBtn?.addEventListener("click", () => {
  const enabled = settings.toggleEq();
...
ui.normalizeBtn?.addEventListener("click", () => {
  const enabled = settings.toggleNormalize();
...
ui.waveBtn?.addEventListener("click", () => {
  const enabled = settings.toggleWaveform();
...
ui.transitionBtn?.addEventListener("click", () => {
  const mode = settings.cycleTransitionMode();
